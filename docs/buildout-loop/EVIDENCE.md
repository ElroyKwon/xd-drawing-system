...
## S12 — 이슈 라이프사이클 이메일 알림(mock) → DONE (prompts/16 FROZEN, 세션17 2026-07-03)

> S11 위. 이슈 생성/상태변경 → 구독자(프로젝트 구성원) mock 알림. 독립 검증자 Q1~Q7 PASS.

### 구현
- `backend/notifications.py`: `subscribers_for`(프로젝트 구성원 이메일, actor 제외, 미구성 폴백)·`notify_issue_event(kind, issue, project, actor)`(email_service 반복, per-recipient try/except)·`enabled()`(XD_NOTIFY).
- `backend/routes_issue.py`: create 후 `notify("created", actor=현재사용자)`, patch status 실변경 시 `notify("status_changed", actor=현재사용자)` — 둘 다 훅 try/except 방어.

### 독립 검증자 채점 (Q1~Q7 전항목 PASS, BLOCKER/MAJOR 0)
| Q | 판정 | 근거 |
|---|---|---|
| Q1 생성 알림 | PASS | 생성→outbox +3(viewer/reviewer/owner), issue_created |
| Q2 상태변경(실변경만) | PASS | 열림→진행중 +3, 동일값·비-status PATCH +0 |
| Q3 구독자 스코프·actor제외 | PASS | 비구성원 누출0, 이메일보유자만 |
| Q4 실패 격리 | PASS | 3중 방어(루프·훅·영속 선행), test_notify_failure_isolated |
| Q5 토글 | PASS | XD_NOTIFY=0 미발송 |
| Q6 외부 egress 0 | PASS | mock outbox만 |
| Q7 회귀0 | PASS | backend 109·격리 무영향 |
- 중복/폭주 없음(구독자 수만큼 정확히 1회).

### MINOR 수리
- 검증자 MINOR(생성 훅 actor 미배선 → 자기알림) → create·status 훅에 `actor=store.get_current_user()` 배선(생성자 본인 제외). backend 109 유지.

### Done-When(S12) reconcile — 전요소 MET(device), NARROWED/UNMET 0
생성 알림·상태변경 알림·구독자 스코프·실패 격리·토글·외부 egress 0 전부 device 라이브 입증.

### ✅ S12 DONE (mock). 실 발송=GATE-5.

---

## S13 — 실 로그인+매직링크+외부 업로드 [설계/게이트] (prompts/17, 세션17 2026-07-03)

> 밤샘 자율 범위: **설계 + HUMAN_GATE 등재까지**. 실 인증 코드=프로덕션 게이트(GATE-6). 버전추적은 S3 구현완료 → 실 인증만 접붙이는 설계.

### 산출물 (설계 스테이지 R1~R3 충족)
- **설계 문서** `prompts/17-s13-auth-design.md`: 현 S7 모의 한계 → 실 인증 목표(세션토큰·매직링크·외부업로드) → 설계(매직링크 플로우=S11 이메일 재사용, 외부업로드=S3 버전추적 재사용, 마이그레이션=`get_current_user()` 전역→세션만 교체·RBAC 보존).
- **GATE-6 등재** `HUMAN_GATE.md`: 프로덕션 인증 방식·세션보안·외부노출·이메일 egress(GATE-5 의존)·개인정보 정책 = 사용자 결정.
- **코드 미구현(의도)**: 실 세션·토큰·비밀번호·SSO·외부노출 전부 GATE-6 이후. 자율 범위는 설계까지.

### 상태: 설계 DONE. 실 인증 구현 = HUMAN_GATE(GATE-6, 사용자 결정 대기).

---

## S14 — PDF 시트 ↔ DWG 소스 매칭 + 버전 토대 Phase 1 [DONE] (prompts/19 FROZEN, 세션20 2026-07-06)

> ai-loop 풀루프. 세트 제출(DWG+PDF) → 수동 매핑(N:M) → publish 시 `sheet_key`+`rev` 확정 → 시트↔소스 DWG 왕래. 기존 drawing/sheet/folder/version_set 무변경 + 새 JSON 2개(`_packages.json`·`_sheet_sources.json`) 외부 조인.

### 구현 요약
- **백엔드**: `store.py` package/sheet_source CRUD(ABC + JsonDrawingStore + TypeDBDrawingStore `_MIRROR` 순수 위임) + `next_rev`(프로젝트 스코프·스프레드시트식 A…Z,AA…) + rev 시퀀스 헬퍼(`_rev_to_index`/`_index_to_rev`). `routes_package.py` 신설(`/api/packages` CRUD·files 귀속[형식 재분류]·hints[`_normalize` 재사용]·mapping draft 저장·publish[발급/계승·is_current 내림·loose 요약]·`/api/sheet-sources`). `main.py` 라우터 2개 등록. `schema/04-drawings.tql` package·sheet_source entity.
- **프론트**: `src/api/packages.ts`(클라이언트) · `src/build/package/`(`mappingState.ts` 순수전이 + `SheetSourceMapper.tsx` 매핑화면[HTML5 DnD + a11y 버튼 경로 + 힌트칩 + 계승 셀렉트 + 변환 폴링·수동 새로고침] + `PublishSetModal.tsx` 세트 업로드→패키지 + `package.css`). `FilesView.tsx` "세트 발행" 버튼 + "세트 목록"(draft 재오픈). `SheetViewerShell.tsx` "소스 DWG 열기"(dedupe·팝오버) + `onOpenSheet` + **renderEngine 시트전환 리셋**. `BuildSheetsView.tsx` onOpenSheet 배선.

### 게이트 (전부 GREEN, 8000 내리고)
- backend pytest **125**(S14 16 = 코어 10 + 3렌즈 수리 회귀 6, 기존 회귀 0) · frontend vitest **128**(S14 8 = mappingState 5 + gating 3) · `npm run build` PASS · `git diff --check` clean.

### 독립 3렌즈 검증 + 수리
- **렌즈1(백엔드 적대)**: BLOCKER 0. **MAJOR-1**(sheet_key 계승/`next_rev`/`is_current` 강등이 프로젝트 경계 무시) + MINOR-2(미존재 키 계승→신규발급)·MINOR-3(rev 27+ 소진 중복 AA)·MINOR-4(미지형식 PDF 오분류)·MINOR-5(orphan sheet 발행) — 6개 반례 CONFIRMED. **전량 수리+회귀 6**(계승=동일 프로젝트 실재 키만 허용 400·`next_rev` 프로젝트 스코프+스프레드시트 시퀀스·형식 dwg/dxf/pdf만·orphan skip).
- **렌즈2(프론트 a11y/비기능)**: BLOCKER/MAJOR 0, MINOR 5. a11y 이중경로·canEdit 게이팅·N3 정합·클린업 PASS 확인. **수리**: MINOR-1(변환 폴링 영구정지→DWG 레이아웃까지 대기+수동 새로고침+소진후 발행허용)·MINOR-2(초기로드 전 저장 가드)·MINOR-3(소스링크 dedupe)·MINOR-5(중복 import 병합). MINOR-4(발행실패 고아업로드)=Phase1 허용 노트.
- **렌즈3(Done-When 비평)**: N1~N9 전항목 MET, NARROWED/UNMET 0, HUMAN_GATE 불필요. 경계노트2 처리: (1) 계승 셀렉트 UI 미노출 → **매퍼에 계승 셀렉트 추가**(후보 존재 시만 노출, Phase1 빈 케이스 자연히 무표시). (2) "전체 스위트 선재실패 4건" → **검증자가 e2e 서버 띄운 채 실행한 아티팩트**로 확정(8000 내린 재실행: pytest 125·vitest 128 전부 green).

### 자체 e2e 적발·수리 실버그
- **renderEngine stale**: PDF→소스 DWG 왕래가 셸을 remount 안 해 렌더엔진이 "raster"로 새어 VectorCanvas 미마운트(canvasCount 0). 시트 전환 effect에 `setRenderEngine(vectorCapable?…)` 추가 → 수리 후 canvasCount 1 device 확인.
- **변환 폴링**: 매퍼가 PDF 시트만 기다려 DWG 레이아웃 0으로 멈춤 → `isConverting`이 DWG 레이아웃까지 대기하도록 수리.

### 브라우저 e2e (device, 격리 임시 uploads·독립 Chrome 9223 CDP, 콘솔 0)
합성 DXF(paperspace 레이아웃 EE-01-000/001) + 2p PDF로, ODA 미설치 우회. 한 번의 풀패스로:
- **N1**: 세트 발행 모달에 DXF+PDF 2파일 스테이징 → "세트 만들고 매핑" → 패키지 draft 생성.
- **N9/N2**: 매퍼 PDF 시트카드 2 + DWG 레이아웃 2, **a11y 버튼 경로**(시트 선택→"지정")로 매핑 1(배지 "매핑됨"·링크칩 1). 임시저장→닫기→"세트 목록" draft 재오픈 → **매핑 복원**(링크칩 1·매핑배지 1).
- **N4/N5**: 발행 → 요약 "발행 링크 1 · 미매핑 시트 1 · 미링크 DWG 0"(page2 loose 허용).
- **N3**: 시트 목록→PDF 시트(EE-01-000, 래스터) 열기→"소스 DWG 열기"→ 부제 `electrical-set`·벡터 렌더토글 등장·**canvasCount 1**(DXF 벡터 뷰어 왕래 성공).
- **콘솔 에러 0** 전 과정. 증거 `evidence/s14-01~08.png`(files·set-upload·mapper·mapped·restore·published·pdf-viewer·source-dwg).

### Done-When(S14) reconcile — N1~N9 전항목 MET, NARROWED/UNMET 0
| N | 판정 | 증거등급 | 근거 |
|---|---|---|---|
| N1 세트→패키지+분류 | MET | device+emulated | e2e 2파일→패키지 / pytest 형식 재분류·미지형식 400 |
| N2 매핑 영속·리로드 | MET | device | e2e 재오픈 복원(링크칩 1) / pytest save-restore |
| N3 소스 DWG traversal | MET | device | e2e traversalVectorCanvas true·sub=electrical-set |
| N4 sheet_key+rev 발급/계승 | MET | device(발급)+emulated(계승) | e2e 발행 링크1 / pytest A→B·is_current 내림·프로젝트 스코프. 계승 셀렉트 UI 노출 |
| N5 loose 시각구분+요약 | MET | device | e2e "미매핑 1" 요약·배지 / pytest 전량 미매핑 발행 |
| N6 직교성·무회귀 | MET | static+emulated | 순수 additive diff / pytest 125·vitest 128 회귀 0 |
| N7 백엔드 영속+RBAC | MET | emulated | pytest RBAC 403·미러 위임·schema entity |
| N8 테스트 게이트 | MET | emulated | pytest 125·vitest 128·build·diff-check clean |
| N9 브라우저 e2e+콘솔0 | MET | device | 8 스크린샷·콘솔 0·a11y 버튼 경로 실동작 |

### ✅ S14 DONE. Out of scope(자동변환·supersede diff/verify·이슈↔해결버전 연결)=Phase 2. 빌드아웃 10/10+S14.
