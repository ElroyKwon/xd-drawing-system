...
### GATE-7 [신규·S15 업로드 지능화] — 도면 내용 **대량** 외부 LLM 전송 (미해결)
- **문제**: S15(백본1)는 업로드 시 도면에서 본문·설비 태그를 자동 추출한다. LLM 트랙(8002)을 켜면 **업로드되는 모든 도면 내용이 외부 LLM(OpenAI)으로 전송**된다. GATE-4(챗 egress 승인)는 **사용자가 질문할 때 그 맥락만** 나가는 것이었다 — **업로드 전량 자동 전송은 규모·성격이 다른 별개 결정**이다.
- **현 상태(자율 구현 범위)**: 킬스위치 `XD_EXTRACT_LLM=0`이 **기본**. 기본 동작은 **규칙기반 추출만**(8000, egress 0). 8002를 기동하지 않아도 시스템 100% 동작(합격기준 O4·O11). provider=mock이면 egress 0.
- **사용자 결정 필요**: ① 실 고객 도면(청주 40장 등)을 외부 LLM에 대량 전송해도 되는가 ② 되면 어느 제공자·모델·리전 ③ 전송 범위(텍스트만/이미지 포함) ④ 감사·보존 정책(S8.4 egress 감사로그 재사용) ⑤ 로컬 LLM 대안(Ollama) 검토 여부.
- **의존**: S8.4 egress 감사 인프라. 미해결 상태에서도 **S15 구현·검증은 규칙기반 트랙으로 진행 가능**(DONE 선언 가능). 실 LLM 켜기만 게이트.

---

### GATE-6 [신규·S13 프로덕션 인증] — 실 로그인/매직링크/외부 노출 (미해결)
- **문제**: S13 설계로 실 로그인·매직링크 초대·외부 업로드 플로우를 정의했으나, **실 자격증명·세션 보안·프로덕션 인증 도입은 외부 노출·보안 경계**라 자율 진행 금지. 현 S7은 로컬 모의(서버 전역 가변 사용자, 보안 경계 아님).
- **현 상태(자율 산출)**: 설계 문서 `prompts/17-s13-auth-design.md`. 버전추적은 S3 구현 완료, RBAC/역할모델은 S7 실동작 — 바뀌는 것은 `get_current_user()` 소스(전역→세션 토큰)뿐. 실 인증 코드는 **미구현**(게이트).
- **사용자 결정 필요**: ① 인증 방식(매직링크/비밀번호/SSO·IdP) ② 세션 보안(서명키·만료·쿠키 속성·CSRF) ③ 외부 노출(HTTPS·배포·레이트리밋 — LOOP "외부 배포"·"클라우드 전환" 게이트 연동) ④ 매직링크 이메일 발송(GATE-5 의존) ⑤ 개인정보 보관/삭제 정책.
- **의존**: GATE-5(이메일 egress) → 매직링크 발송. LOOP.md "인증/RBAC 프로덕션 적용·외부 배포·클라우드 백엔드 전환" 게이트.
