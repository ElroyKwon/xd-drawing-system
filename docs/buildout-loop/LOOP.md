...
- [~] **AI 사이드카 챗(XD 추가 역량)**: 격리 8001 사이드카가 8000 공개 HTTP만으로 프로젝트 실데이터를 그라운딩해 실 LLM(gpt-5.5) Q&A. → **구현 진행 중**: S8.0/S8.1/S8.3 DONE(부트스트랩·두뇌·앱 드로어), 실 GPT 라이브 실증. **잔여 S8.2(툴 카탈로그+이밸)·S8.4(egress 감사)·S8.5(3렌즈 검수+reconcile)** 완료 시 DONE 선언. 상세 `ROADMAP.md` §3.
