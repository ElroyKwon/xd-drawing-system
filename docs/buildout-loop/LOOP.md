...
- [x] **XD 고유(온톨로지)**: 도면 entity가 TypeDB에 적재되고 `equipmentEntityId` 바인딩이 동작(Study_TypeDB `analysis_result` 계승). AI 분석은 별도 단계. → **DONE(2026-07-03 세션17, S10)**: equipment 온톨로지 TypeDB 실적재 + `appears_on` 시트 바인딩(=equipmentEntityId 계승, 관계44) + 사이드카 AI TypeDB 그라운딩. 독립 검증자 O1~O8 PASS. **equipment 출처=큐레이트 시드**(AI 자동추출은 freeze상 별도 후속 단계). 증거 `EVIDENCE.md` S10.
