...
## 세션 27 (2026-07-09) — AI 지식그래프 트랙: 스펙 FROZEN + 구현계획 ✅ (문서만)

**브레인스토밍 재개 → 스펙 FROZEN → writing-plans 완료.** 코드 변경 없음(설계·계획 문서만).

- **§2 승인 3점 확정**: ①단계10 overlay 완전 되돌림 ②설비 노드=참조 투영(ref_id, 복제0) ③relates_to·note AI 소유 — track(curated/rule/llm)으로 정직성 충분 + **relates_to 승격 경로**(llm→curated) 단, 모델·표기만 첫 스펙·확인 쓰기는 ⑥ write-back 이연.
- **§3~§7 확정**: 통합 스냅샷+멱등 재빌드(`uploads/_knowledge_graph.json`) · **외부 AI API=8002 사이드카 신규 `/analyze`**(사용자 강조: 분석·생성은 반드시 외부 AI API 경유) · 전용 페이지 force 그래프(xg-web `web/nms.js` forceLayout 차용) · overlay 외과적 되돌림(`f0782d9`는 순수 리버트 아님 — overlay만 발라냄).
- **FROZEN 스펙** `docs/superpowers/specs/2026-07-09-knowledge-graph-layer-design.md` (커밋 `696e9bf`).
- **구현계획** `docs/superpowers/plans/2026-07-09-knowledge-graph-layer.md` — TDD **10 태스크**(§6 되돌림→①스토어→②사이드카/analyze+멱등빌드→③조회API+kg_*툴→④전용뷰 force→통합검증), 코드 전량 동봉, self-review 통과 (커밋 `39afffb`).
- **다음 = 실행**: 방식 선택(Subagent-Driven 권장/Inline) 후 **Task 1**(§6 overlay 되돌림)부터. 진입 = 이 계획 문서.
- **불변식 계승**: TypeDB 원칙(지식그래프 SoT=JSON) · AI 사이드카 격리(8002 backend import 0) · **외부 AI API 필수**(HUMAN_GATE-7, 기본 mock=egress 0) · 정직성(track/confidence) · 8000 egress 0 · 회귀 0.

## 세션 28 (2026-07-09) — AI 지식그래프 읽기 트랙 ①②③④ 구현 DONE ✅ (Subagent-Driven)

**계획(`plans/2026-07-09-knowledge-graph-layer.md`) 10 태스크 전량 실행.** Subagent-Driven: 태스크마다 구현 에이전트 → 스펙 검토 → 코드품질 검토 → 수리 → 커밋(로컬, push 미실행). 16 커밋(`a0ebe42`~`5351507`).

- **Task 1 §6 overlay 되돌림**: `_norm_tag`·`_extracted_overlay`·`include_extracted` 외과 제거, `list_equipment` 순수 큐레이트 복원. 되돌림 가드 테스트를 추출태그 시드 **행동검증**으로 강화 + config 전역 누출·sys.path 부트스트랩 수리. (`a0ebe42`+`5e9eb17`)
- **Task 2 `kg_store.py`**: 로드·순회(neighbors/path/evidence BFS)·무결성. 리뷰 수리=**neighbors 유도 서브그래프 엣지 완성**(공동방문 엣지 누락·types 필터 dangling 제거)·`_load` 손상 로깅. (`898458d`+`16e0153`)
- **Task 3 8002 `/analyze`**: mock=공출현 결정적 관계(egress 0)·openai=실 LLM(GATE-7, `_complete` 공용추출, read() 불변). 하드닝=base 계약 선언·프롬프트 상한·except 협소화. **격리 O12 유지**(AST 가드 5 PASS). (`2fb31b2`+`782a61c`)
- **Task 4 멱등 빌드**: 투영(8000 GET)+AI(8002)→`_knowledge_graph.json`, 무결성 게이트로 dangling 거부, built_at 인자주입(시계 0). 리뷰 수리=**note-id builtin `hash()`→sha256 content hash**(멱등 재빌드 보장, 결정성 가드 테스트 추가). (`9740b7f`+`6943dfe`)
- **Task 5 조회 API `routes_kg.py`**: node/neighbors/path/evidence/graph(읽기 전용)+main 등록. `from`→`Query(alias)` 정상. 리뷰 수리=**graph scope 404 정합**+에러경로 4 테스트. (`34b7fed`+`80589a9`)
- **Task 6 kg_* AI 툴**: kg_neighbors/kg_path/kg_evidence, 8000 GET만(격리). 리뷰 수리=**404→{found:false} 정직 처리**(get_equipment 관례)+테스트 import 독립화. (`171dc86`+`e477f92`)
- **Task 7 `src/api/kg.ts`**: fetchGraph/fetchNeighbors, `drawings.ts` BACKEND_BASE 공유 규약. 타입 계약 정확(ref_id/evidence null·track 리터럴). ✅ Approved. (`01cf02e`)
- **Task 8 `KnowledgeGraphView.tsx`+`kgForce.ts`**: 결정적 force 레이아웃(xg-web 이식, 난수/시계 0, 매반복 bounds 클램프)+canvas 뷰(노드색=type·엣지 track curated 실선/llm 점선). 리뷰 수리=**projectName 전환 시 stale graph/error 리셋**. (`6114985`+`3f9436e`)
- **Task 9 App 네비 배선**: `activeView` 유니온+지식그래프 탭. 조기 return 아닌 **nav 레벨 peer**로 배치(tsc TS2367로 union narrowing 포착). ✅ Approved. (`5351507`)

**Task 10 통합 검증**:
- **회귀 종합 GREEN**: 프론트 vitest **135**(22파일) · 백엔드 tests/ **196**(+1skip) · AI 사이드카 **50** · 8002 extract **8** · `npm run build` GREEN · tsc clean.
- **실 빌드**(project "LS 청주사업장", mock egress 0): **134 nodes · 141 edges**, `_knowledge_graph.json` 영속, built_at 정확. node types(eq15·sheet40·tag23·issue10·task6·file40)·edge types(has_tag59·appears_on33·pinned_to9·references40)·tracks(rule108·curated33).
- **조회 API 실데이터 스모크**: graph/neighbors(eq:EQ-ACB-LV,depth2)/evidence 전부 **200**. 증빙 `evidence/kg-integration-smoke-260709.txt`.
- **⚠️ relates_to(llm)=0 — 정상 동작**: 설비 큐레이트태그(MTR-1·VCB-22.9KV…) ∩ 시트 추출태그(LV-6·TR-1201…) **겹침 0**(naming 분리). mock `/analyze`는 70개 시트태그 관계 반환하나 설비노드 미매핑으로 전량 드롭(**설비관계 날조 방지 = 설계대로**). 실 설비 relates_to는 도면 의미해석 **GATE-7 LLM** 몫. → 후속 결정: 오프라인 데모용으로 mock을 "같은 시트 공존 설비쌍"으로 확장할지(설계 변경, 사용자 결정).
- **⚠️ 브라우저 e2e(canvas 픽셀 페인트) 미실행**: 환경 제약(chrome-devtools 프로필 잠금 + wmux 미기동). 렌더 경로는 vitest 컴포넌트 렌더 테스트 + kgForce 결정성/bounds 테스트 + 무조건 클램프로 커버, CORS allow-credentials 활성. dev 5178 라이브(육안 확인 가능). 실 canvas 페인트만 미검증.

- **⑤⑥ write-back 이연**(스펙대로): relates_to 확인 승격·증분 write-back은 다음 트랙.
- **HUMAN_GATE 유지**: GATE-5(실 이메일)·GATE-6(실 인증)·**GATE-7(8002 실 LLM egress)**·미푸시 커밋 누적(origin 대비 ahead, push 사용자 승인 대기).
- **다음 진입점**: ①브라우저 e2e 재시도(환경 정리 후) 또는 육안 확인 ②relates_to 데모 mock 확장 여부 결정 ③⑤⑥ write-back 트랙 ④미푸시 push 결정.

---

## 세션 29 (2026-07-09) — ⑥ write-back 설계 스펙 FROZEN ✅ (브레인스토밍, 문서만)

세션28에서 사용자가 세션29 트랙으로 **⑤⑥ write-back**을 선택. 착수 전 브레인스토밍으로 발견한 두 핵심:
1. **⑤⑥은 계획뿐 아니라 스펙조차 없었다** — 읽기 스펙이 §2③·§5에서 "다음 스펙"으로 명시 이연. → 세션27 방식(브레인스토밍→스펙 FROZEN→writing-plans→구현) 계승해 **스펙부터** 시작.
2. **심장 긴장 = 멱등 재빌드가 승격을 지운다** + **현실 함정 relates_to(llm)=0**(승격할 데이터 없음). 이 둘이 설계를 규정.

**브레인스토밍 4결정(사용자 승인)**:
- W1 범위 = **메커니즘만(데이터 불문)** — 임의 llm 엣지 안전 승격/거부만. 실 relates_to 데이터는 mock확장/GATE-7 독립 트랙.
- W2 아키텍처 = **오버레이 저널 `_kg_overlay.json`(append-only) + 로드타임 병합**. 빌드는 오버레이 모름(멱등 유지) → 승격 재빌드 생존.
- W3 동작 = **confirm(llm→curated) + reject(drop)**. track=llm 엣지만 대상(rule·curated 보호). 수동추가·note편집 이연.
- W6 UI = **최소 UI 포함** — canvas llm 점선 클릭→confirm/reject 버튼→track 즉시 육안. relates_to(llm)=0이라 시연용 시드 스크립트로 llm 엣지 1개 주입.
- 스코프 밖(명시 이연): 증분 재빌드(YAGNI)·수동추가·note편집·실인증(GATE-6)·mock확장·GATE-7·⑤ 서비스라우팅.

**산출**: FROZEN 스펙 `docs/superpowers/specs/2026-07-09-kg-writeback-design.md`(10장, 커밋 `b9b7f07`). 코드 무변경 → 회귀 기준선 세션28과 동일(vitest 135·pytest 196·AI 50·8002 8).

- **다음 진입점(세션30)**: 이 스펙으로 **writing-plans 구현계획 작성** → subagent-driven 구현(오버레이+병합+쓰기 라우트 신규파일+최소 UI+시드 스크립트+테스트). 스펙 §10 테스트 전략이 Done-When.
- **HUMAN_GATE 유지**: GATE-5·6·7 + 미푸시 커밋 26개(push 사용자 승인 대기).

---

## 세션 33 (2026-07-10) — B 설비관계 + A 멀티모달 비전 + P1/P2 대화형 액션 ✅ (전부 push)

> (세션30~32는 브릿지 §2가 현행 SoT였고 PROGRESS는 세션29에서 멈춰 있었음 — 이 블록이 그 이후를 종합.)

**전부 origin push 완료. HEAD `ffac979`.** 이번 세션에서 사용자 요청 트랙 전량 구현·검증.

- **B 데이터트랙 Stage1** (설비 `appears_on` 공존→relates_to(llm)): provider 재작성 + build sheet_ids + 매핑 e2e. `relates_to(llm)` **0→16** 실측. KG 45 green. 커밋 `fcf9f7d`·`2d78122`·`fa764f9`.
- **A 멀티모달 비전 분석** (사용자 지시: text+image·상한 제거): `XD_KG_VISUAL=1` 게이트. 시트 PDF→`fitz`(텍스트+페이지 렌더)→**gpt-5.6-terra** 멀티모달(`/analyze_sheet`)→feeds/protects/relates_to·정격 포함 설비(eq:llm:*)·노트를 llm 분리레이어로. **실측 청주 41/41장: 1717노드(설비1499·노트97)·feeds1027·protects275·relates_to228·describes97, 무결성0**. 커밋 `15065a8`(+effort 배선 `761e5a9`). 텍스트만 148노드 → 비전 1717.
  - **★모델 실측 교정**: 문자열은 `gpt-5.6-terra`(r 2개, "tera" 아님). effort는 `normal` **API 미지원**(none/minimal/low/medium/high/xhigh) → **medium** 사용. 반드시 `openai`+`pymupdf` 있는 venv(`backend/ai/.venv`)로 8002 기동.
- **용어**: "지식그래프"→**"메타그래프"** (UI 표시텍스트만). 커밋 `d6c982e`.
- **P1/P2 대화형 액션** (제안→확인카드→실행→감사, subagent-driven): 사이드카(8001) `propose_*`는 8000 mutate 0(격리 불변식 AST+respx 검증). 확인카드[실행]→기존 src/api 뮤테이터+감사(origin=ai_chat). 커밋 `ffac979`. 검증(직접): 사이드카55·감사1·ActionCard2·build clean + **브라우저 라이브 e2e**(gpt로 이슈 제안→[실행]→LV-3501 이슈 실생성 id `e154d8e4`→감사 POST/GET 실기록). Task8 device-e2e 충족.
- **계획 전수 감사**(서브에이전트): 남은 owed = ⑤ 라우팅 인덱스(이연) · Stage2 방향성 관계(**비전 feeds/protects로 실질 달성**) · 발표자료.

### ▶▶ 다음 세션(34) 진입점 — **"개발 완료 선언 가능한가?" 판단 (발표자료 제외)**
사용자 지시(세션33 종료): "**발표자료 하는 일 빼고, 이 프로젝트 개발 완료를 선언할 수 있는지** 다음 세션에서 그것부터 보자." → 다음 세션 첫 작업 = 전체 스펙/플랜/Done-When 대비 **완료 판정 감사**(무엇이 done·무엇이 남았나·HUMAN_GATE만 남았나). 발표자료 챕터10 재촬영은 이 판정에서 제외.

- **잔여(비완료 후보)**: ⑤ 서비스·툴 라우팅 인덱스(스펙·코드 0) · 실 인증/이메일(GATE-5/6) · routes_issue/task origin 파라미터(현재 dead code라 미추가·감사는 전용엔드포인트로 충족) · 프론트 canvas jsdom 테스트 1건 pre-existing 실패(react-force-graph).
- **HUMAN_GATE 유지**: GATE-5(실 이메일)·GATE-6(실 인증/SSO/배포). GATE-7(실 LLM 추출)은 이번 세션 사용자 승인·실행됨.
